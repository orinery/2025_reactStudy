...
function _getDiscountedMenus(menus) {
  menus.forEach((menu) => {
    const discountedPrice = menu.price - 500;
    console.log(
      `itemId: ${menu.itemId}, name:'${menu.name}, price: '${menu.price}, discountedPrice: ${discountedPrice}'`
    );
  });
}

function getDiscountedMenus(menus) {
  return menus.map((menu) => ({ ...menu, discoundtedPrice: menu.price - 500 }));
}
